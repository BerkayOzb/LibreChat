...
import { X, UserX, Loader2, AlertTriangle, UserMinus } from 'lucide-react';
import { useLocalize } from '~/hooks';

interface RemoveMemberFromOrgModalProps {
    isOpen: boolean;
    onClose: () => void;
...
export default function RemoveMemberFromOrgModal({
    isOpen,
    onClose,
    onConfirm,
    userName,
    isLoading = false,
}: RemoveMemberFromOrgModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError(err?.message || localize('com_admin_remove_member_failed'));
        }
    };
                ...
                <div className="inline-block transform overflow-hidden rounded-xl bg-surface-primary text-left align-bottom shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md sm:align-middle border border-border-medium">
                    ...
                    <div className="flex items-center justify-between px-6 py-4 border-b border-border-medium">
                        <h3 className="text-lg font-semibold text-text-primary flex items-center gap-2">
                            ...
                            {localize('com_admin_remove_member_title')}
                        </h3>
                        <button
                            onClick={onClose}
                                ...
                                <div className="flex gap-3">
                                    ...
                                    <div>
                                        <p className="text-sm text-amber-800 dark:text-amber-200">
                                            {localize('com_admin_remove_member_warning')}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mb-4 p-4 rounded-lg bg-surface-secondary border border-border-medium">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                        <div className="font-medium text-text-primary text-lg">{userName}</div>
                                    </div>
                                </div>
                            </div>

                            <p className="text-sm text-text-secondary mb-4">
                                {localize('com_admin_remove_member_description')}
                            </p>
                                ...
                                >
                                    {isLoading ? (
                                        ...
                                        <>
                                            ...
                                            {localize('com_admin_remove_member_button')}
                                        </>
                                    )}
                                </button>
                            </div>

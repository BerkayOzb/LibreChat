...
/**
 * Ensure default agents exist (run on startup)
 * Creates "Görsel Üretici" and "Borsa Asistanı" agents if they don't exist
 * @returns {Promise<{created: string[], existing: string[]}>}
 */
const ensureDefaultAgents = async () => {
  try {
    logger.info('[ensureDefaultAgents] Checking default agents...');
    ...
    let systemUser = await User.findOne({ email: 'system@veventures.local' });

    if (!systemUser) {
      logger.info('[ensureDefaultAgents] Creating system user...');
      ...
      logger.info('[ensureDefaultAgents] System user created');
    }

    // **1. Görsel Üretici Agent**
    const visualAgentExists = await Agent.findOne({
      name: 'Görsel Üretici',
      author: systemUser._id,
    });

    if (!visualAgentExists) {
      logger.info('[ensureDefaultAgents] Creating Görsel Üretici agent...');

      const visualAgentId = 'agent_' + crypto.randomBytes(12).toString('base64').replace(/[+/=]/g, '').substring(0, 21);
...
- Sistem otomatik olarak hangi tool'ların gerekli olduğunu belirleyecek`,
        tools: ['nano-banana'],
        autoToolFilter: true,
        availableTools: ['nano-banana', 'flux', 'dalle'],
        capabilities: ['tools'],
        temperature: 0.7,
        top_p: 1,
        presence_penalty: 0,
      ...
      created.push('Görsel Üretici');
      logger.info('[ensureDefaultAgents] ✅ Görsel Üretici agent created');
    } else {
      // Update existing agent with critical fields if missing
      await Agent.updateOne(
        { name: 'Görsel Üretici', author: systemUser._id },
        {
          $set: {
            autoToolFilter: true,
            availableTools: ['nano-banana', 'flux', 'dalle'],
      ...
      existing.push('Görsel Üretici');
      logger.info('[ensureDefaultAgents] ✓ Görsel Üretici agent already exists (fields synced)');
    }

    // **2. Borsa Asistanı Agent**
    const borsaAgentExists = await Agent.findOne({
      name: 'Borsa Asistanı',
      author: systemUser._id,
    });

    if (!borsaAgentExists) {
      logger.info('[ensureDefaultAgents] Creating Borsa Asistanı agent...');

      const borsaAgentId = 'agent_' + crypto.randomBytes(12).toString('base64').replace(/[+/=]/g, '').substring(0, 21);

      // All borsa-mcp tools (same as migration)
      const borsaMcpTools = [
        ...
        'get_btcturk_ticker_mcp_borsa-mcp',
        'get_btcturk_orderbook_mcp_borsa-mcp',
        'get_btcturk_trades_mcp_borsa-mcp',
        'get_btcturk_ohlc_mcp_borsa-mcp',
        'get_btcturk_kline_mcp_borsa-mcp',
        'get_btcturk_technical_mcp_borsa-mcp',
        ...
        'get_coinbase_technical_mcp_borsa-mcp',
...
Kullanıcılara yardımcı ol, finansal okuryazarlıklarını artır!`,
      ...
      created.push('Borsa Asistanı');
      logger.info('[ensureDefaultAgents] ✅ Borsa Asistanı agent created');
    } else {
      // Update existing agent with critical fields if missing
      await Agent.updateOne(
        { name: 'Borsa Asistanı', author: systemUser._id },
        {
          $set: {
            mcp: ['borsa-mcp'],
            capabilities: ['tools'],
      ...
      existing.push('Borsa Asistanı');
      logger.info('[ensureDefaultAgents] ✓ Borsa Asistanı agent already exists (fields synced)');
    }

    logger.info('[ensureDefaultAgents] Summary:', {
      created: created.length > 0 ? created : 'none',
      existing: existing.length > 0 ? existing : 'none',
    });

    return { created, existing };
  } catch (error) {
    logger.error('[ensureDefaultAgents] Error ensuring default agents:', error);
    throw error;
  }
};

...
const getOrganizationById = async (req, res) => {
    try {
        ...
        const admins = await User.find({ organization: id, role: SystemRoles.ORG_ADMIN }).select('name email _id').lean();
...
const deleteOrganization = async (req, res) => {
    try {
        ...
        const userCount = await User.countDocuments({ organization: id });
        if (userCount > 0) {
            return res.status(400).json({ message: 'Cannot delete organization with existing users. Please remove users first.' });
        }
...
const assignOrgAdmin = async (req, res) => {
    try {
        ...
        if (user.role === SystemRoles.ADMIN) {
            return res.status(400).json({ message: 'Cannot assign Global Admin to an Organization. Demote them first if necessary.' });
        }
...
const getOrganizationUsers = async (req, res) => {
    try {
        ...
        const total = await User.countDocuments({ organization: id });
